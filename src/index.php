<!doctype html>
<html lang="en">
  <!DOCTYPE html>
  <html>
  <head>
    <title>Sushi Kumo</title>
    <link rel="stylesheet" href="Principal.css">
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Josefin+Sans:wght@400;700&family=Poller+One&display=swap">
  </head>
  <body>
    <div class="wrap-header-hero">
      <header class="main-header">
        <div class="header-wrap">
          <div class="wrap-logo-header">
            
            <a class="logo-header" href="/">
              Sushi Kumo
            </a>
          </div>
          <div class="wrap-nav-header">
            <nav class="nav-header">
              <input type="checkbox" id="check">
              <label for="check" class="checkbtn">
                <i class="toggle-menu">
                  <img src="data:image/svg+xml;base64," />
                </i>
              </label>
      
              <ul class="main-menu">
                <li class="menu-item"><a href="">Inicio</a></li>
                <li class="menu-item"><a href="Menu.php">Menu</a></li>
                <li class="menu-item"><a href="Contacto.php">Contacto</a></li>
                
            
                <li id="nav-main-contact" class="menu-item"><a href="Registro.php">Iniciar sesion</a></li>
              </ul>
      
            </nav>
          </div>
      
       </header>
      
        <main class="main-section">
          <!--Hero seccion-->
          
          <section class="hero-home-page">

          <div class="wrap-hero-home-page">
            <h1>BIENVENIDOS</h1>

            <p>Sushi Kumo es un negocio de comida japonesa, la cual ofrece platillos como sushi, ramen, yashioka, entre otros.
               Elaborados con parte de la esencia mexicana, buscando ser más inovadores!.
            </p>

            <a href="" class="cta-main">
              Haz tu pedido!
 </a>
          </div>
          </section>
        </div>
      <!--Menu de platillos-->
      <section class="wrap section">
        <div class="wrap-title-section">
          <h2>Algunos Platillos</h2>
        </div>
        <div class="column-4 colums">
          <div class="tarjetas-rest">
            <div class="wrap-text_tarjeta-charola">
              <h3>Charola familiar</h3>
              <p>  - </p>
              <div class="cta-wrap_tarjetas-rest">
                <div class="precio_tarjeta-rest">
                  <span>$</span>
                </div>
                <div class="cta-tarjeta-rest">
                  <a href="">Pedir ahora</a>
                </div>
              </div>
            </div>
          </div>
          
          <div class="tarjetas-rest">
            <div class="wrap-text_tarjeta-ramen">
              <h3>Rayito ramen</h3>
              <p>  - </p>
              <div class="cta-wrap_tarjetas-rest">
                <div class="precio_tarjeta-rest">
                  <span>$</span>
                </div>
                <div class="cta-tarjeta-rest">
                  <a href="">Pedir ahora</a>
                </div>
              </div>
            </div>
          </div>
          <div class="tarjetas-rest">
            <div class="wrap-text_tarjeta-sushi">
              <h3>sushi empanizado</h3>
              <p>  - </p>
              <div class="cta-wrap_tarjetas-rest">
                <div class="precio_tarjeta-rest">
                  <span>$</span>
                </div>
                <div class="cta-tarjeta-rest">
                  <a href="">Pedir ahora</a>
                </div>
              </div>
            </div>
          </div>
          
          <div class="tarjetas-rest">
            <div class="wrap-text_tarjeta-yakimesi">
              <h3>yakimesi</h3>
              <p>  - </p>
              <div class="cta-wrap_tarjetas-rest">
                <div class="precio_tarjeta-rest">
                  <span>$</span>
                </div>
                <div class="cta-tarjeta-rest">
                  <a href="">Pedir ahora</a>
                </div>
              </div>
            </div>
          </div>

        </div>
      </section>
      
        </main>
      
        <footer>
          <p>&copy; 2022 Sushi Kumo. Todos los derechos reservados.</p>
      </footer>
   </body>
  </html>
  
</html>
